...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
	...
	for(int i = 0; i < pWorld->numOfObjects; i++)
	{
		...
		for(int j = 0; j < pWorld->pObject[i].numOfVerts; j++)
		{
			...
			currentWidth  = abs(pWorld->pObject[i].pVerts[j].x - m_vCenter.x);	
			currentHeight = abs(pWorld->pObject[i].pVerts[j].y - m_vCenter.y);		
			currentDepth  = abs(pWorld->pObject[i].pVerts[j].z - m_vCenter.z);
